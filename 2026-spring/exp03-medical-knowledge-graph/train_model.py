...
# 打印当前工作目录和sys.path路径，确保utils目录被正确添加
print("当前工作目录:", os.getcwd())  # 打印当前工作目录
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), 'utils')))
print("当前sys.path:", sys.path)  # 打印sys.path，确保utils路径在其中

# 导入 utils 中的 DataProcessor
from utils.data_processor import DataProcessor  # 导入 DataProcessor
from models.entity_extractor import MedicalEntityExtractor
import torch


def init_models(model_name="Qwen/Qwen3-4B-Instruct-2507"):
    """
    初始化所有需要的模型
...
def main():
    ...
    output_dir = r"C:\Users\lh008\Desktop\exp3\exp03-medical-knowledge-graph\data\processed"  # 处理后的数据目录
    ...
    for idx, article in enumerate(articles[:max_articles]):
        print(f"\n{'=' * 50}")
        print(f"处理第 {idx + 1}/{min(len(articles), max_articles)} 篇文章")
        print(f"{'=' * 50}")
        ...
        if entities:
            ...
            processed_article = {
                "id": article["_id"],
                "translated": article["description"],  # 直接使用原文作为翻译结果
                **entities
            }
            processed_articles.append(processed_article)
            print(f"\n成功处理文章: {article['description']}")
            print(
                f"提取的实体数量: 症状({len(entities['symptoms'])}), 疾病({len(entities['diseases'])}), 药物({len(entities['drugs'])}), 检查({len(entities['checks'])})")
        else:
            print(f"\n跳过文章: {article['description']} (未提取到实体)")
